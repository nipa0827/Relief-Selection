function [weight,hit,miss] = relieff_complementary(X,Y,xa,K,varargin)
...
weight = NaN(1,p);
hit = NaN(1,size(xa,1));
miss = NaN(1,size(xa,1));
if ~isRegression
    [weight , hit, miss] = RelieffClass(X,xa,yy,C,classProb,numUpdates,K,distFcn,dist1D,sigma);
...
% -------------------------------------------------------------------------
function [attrWeights,hit,miss] = RelieffClass(scaledX,xa,yy,C,classProb,numUpdates,K,...
...
attrWeights = 0;
Nlev = size(C,2);
hit = zeros(1,size(xa,1));
miss = zeros(1,size(xa,1));
...
for i = 1:numUpdates
    ...
    if lenHits>0
        ...
        idxH = knnsearch(searchers{thisC},selectedX,'K',lenHits+1,'Distance','chebychev');
        idxH(1) = [];
...
%     cls = find(thisC);
    ...
    if ~isempty(missClass) % Make sure there are misses!
        ...
        for mi = 1:length(missClass)
            ...
            idxM = knnsearch(searchers{missClass(mi)},selectedX,'K',lenMiss,'Distance','chebychev');
    ...
    hit(i) = diffH(scaledX,thisObs,Hits,dist1D,sigma);
    dM = diffM(scaledX,thisObs,Misses,dist1D,sigma,classProb)/numUpdates;
    miss(i) = diffM(scaledX,thisObs,Misses,dist1D,sigma,classProb);
