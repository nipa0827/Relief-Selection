function [ranked, weight] = relieff_average_average_rank(X,Y,K,varargin)
...
%     %X = bsxfun(@rdivide,bsxfun(@minus,X,min(X)),Xdiff);
% end
...
weight = NaN(1,p);
if ~isRegression
    weight(accepted) = RelieffClass(X,yy,C,classProb,numUpdates,K,distFcn,dist1D,sigma);
...
% -------------------------------------------------------------------------
function attrWeights = RelieffClass(scaledX,yy,C,classProb,numUpdates,K,...
...
classH = zeros(size(scaledX,2),size(unique(yy),1));
classM = zeros(size(scaledX,2),size(unique(yy),1));
...
for i = 1:numUpdates
    ...
    [neighbour,D_neigh]= knnsearch(scaledX,selectedX,'K',lenHits+1,'Distance','cityblock');
    neighbour(1) = [];
    D_neigh(1) = [];
    ...
    tt = find(thisC==1);
     ...
     for j = 1:numAttr 
        ...
        classH(j,tt) = classH(j,tt) + diffH(j,scaledX,thisObs,Hits,dist1D,sigma,classProb(cls));
        classM(j,tt) = classM(j,tt) + diffM(j,scaledX,thisObs,Misses,dist1D,sigma,classProb);
     end
...
for hh = 1:size(classH,1)
    classH(hh,:) =  classH(hh,:)./gc';
end

for hh = 1:size(classM,1)
    classM(hh,:) = classM(hh,:)./gc';
end

attrWeights = -mean(classH,2) + mean(classM,2);
...
function distMeas = diffH(a,X,thisObs,Hits,dist1D,sigma,classProb)
