...
%for dataset = [ 'q' 'j' 's' ')' 'c' 'b' '4' '?' '3' '#' '_' '@' '+' '!' ]
    ...
    for kk=1:no_of_fold
        ...
        fea_mean_original = mean(tr_fea_original);
        fea_diff_original = max(tr_fea_original)- min(tr_fea_original);
        tr_fea = bsxfun(@rdivide,bsxfun(@minus,tr_fea,fea_mean),Xdiff);
        tr_fea_original = bsxfun(@rdivide,bsxfun(@minus,tr_fea_original,fea_mean_original),fea_diff_original);
        %%%%%%%%%%%%%%%%%%%%% Equal Width %%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        ts_fea_original = bsxfun(@rdivide,bsxfun(@minus,ts_fea_original,fea_mean_original),fea_diff_original);
        
        tr_fea1 = tr_fea_original(:,sel_feature);
        tr_label = Sapp(tr_idx);
        ts_fea1 = ts_fea_original(:,sel_feature);
        ts_label = Sapp(ts_idx);
