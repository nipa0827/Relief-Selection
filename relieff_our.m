function [ranked,weight,hit,miss] = relieff_our(X,Y,K,varargin)
...
miss = NaN(size(X,1),p);
if ~isRegression
    [weight(accepted),hit,miss] = RelieffClass(X,C,classProb,numUpdates,K,distFcn,dist1D,sigma);
else
...
% -------------------------------------------------------------------------
function [attrWeights,hit,miss] = RelieffClass(scaledX,C,classProb,numUpdates,K,...
